...
public class ToDoNoticeModelImpl implements ToDoNoticeContract.Model{
    String baseUrl = "";

    public ToDoNoticeModelImpl(String baseUrl){
        this.baseUrl = baseUrl;
    }
    @Override
    public void request(String userId, final ResultListener<ToDoNotifyListEntity> result) {
        Call<ToDoNotifyListEntity> call = AppMainService.getNotifyService(baseUrl).selfList(userId);
        //请求开始
        result.onStart();
        //执行操作
