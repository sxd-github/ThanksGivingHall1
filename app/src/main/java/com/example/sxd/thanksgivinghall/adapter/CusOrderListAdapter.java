...
public class CusOrderListAdapter extends BaseQuickAdapter<CustomerOrderListEntity.Data, BaseViewHolder> {

    private CusOrderListAdapter myAdapter;

    public CusOrderListAdapter(@LayoutRes int layoutResId, @Nullable List<CustomerOrderListEntity.Data> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, CustomerOrderListEntity.Data item) {

        helper.setText(R.id.goodsname,item.getGoods())
                .setText(R.id.sum_goods_price,"总价："+item.getSumprice())
                .setText(R.id.order_name,"客户："+item.getCusname());
