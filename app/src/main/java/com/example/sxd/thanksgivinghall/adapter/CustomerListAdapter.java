...
public class CustomerListAdapter extends BaseQuickAdapter<CustomerListEntity.Data, BaseViewHolder> {

    private CustomerListAdapter myAdapter;

    public CustomerListAdapter(@LayoutRes int layoutResId, @Nullable List<CustomerListEntity.Data> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, CustomerListEntity.Data item) {

        helper.setText(R.id.customer_tit,item.getUsername())
                .setText(R.id.Nation,"民族："+item.getNation())
                .setText(R.id.sex,"性别："+item.getSex());
